...
namespace SQLInjection_Analyzer
{
    ...
    public class SQLInjection_AnalyzerCodeFixProvider : CodeFixProvider
    {
        private const string title = "Make SqlParameter From Variable At SqlCommand";
        private Document newDoc;

        public sealed override ImmutableArray<string> FixableDiagnosticIds
        {
            get { return ImmutableArray.Create(SQLInjection_AnalyzerAnalyzer.DiagnosticId); }
        }
        ...
        public sealed override async Task RegisterCodeFixesAsync(CodeFixContext context)
        {
            try
            {
                ...
                var ObjCreation = root.FindToken(diagnosticSpan.Start).Parent.AncestorsAndSelf().OfType<ObjectCreationExpressionSyntax>().First();
                if (!ObjCreation.Type.ToString().Contains("SqlCommand"))
                {
                    return;
                }
                ...
                ExpressionSyntax expressionSyntax = ObjCreation.ArgumentList.Arguments.First().Expression;
                // Register a code action that will invoke the fix.
                context.RegisterCodeFix(
                    CodeAction.Create(
                        title,
                        c => MakeParameterFromVarAsyn(context.Document, expressionSyntax, c),
                        equivalenceKey: title),
                        diagnostic);
            } catch { }
        }

        private async Task<Document> MakeParameterFromVarAsyn(Document document, ExpressionSyntax expressionSyntax, CancellationToken cancellationToken)
        {
                var binExpression = expressionSyntax as BinaryExpressionSyntax;
                var NewSqlCommand = "";
                ...
                int index = 0;
                bool HaveVar = false;
                string sqlcommandIdentifierName = ((Microsoft.CodeAnalysis.CSharp.Syntax.VariableDeclaratorSyntax)((Microsoft.CodeAnalysis.SyntaxNode)binExpression).Parent.Parent.Parent.Parent.Parent).Identifier.ValueText;
                var tempBlock = ((Microsoft.CodeAnalysis.SyntaxNode)expressionSyntax).Parent.Parent.Parent.Parent.Parent.Parent.Parent.Parent;
